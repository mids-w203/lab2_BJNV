library(tidyverse)
library(haven)
# Read BRFSS .xpt file (change the path to match your system)
brfss <- read_xpt("/Users/nanaedusah/Documents/GitHub/Statistics/Lab2/CDC Files/LLCP2023.XPT ")
view(head(brfss,100))
df <- brfss %>%
  select(`_STATE`,MENTHLTH, INCOME3, LADULT1,EXERANY2, MARITAL, EDUCA,VETERAN3,EMPLOY1) %>%
  filter(MENTHLTH <= 30 | MENTHLTH == 88, INCOME3>=1 & INCOME3 <= 12, 
         LADULT1!= 1) %>%
  mutate(
    MENTHLTH = ifelse(MENTHLTH == 88, 0, MENTHLTH), #if number of mental health days code is none, with code 88, then change the frequency to 0
    INCOME3_label = case_when( #change income level to midpoint of range
      INCOME3 == 1 ~ "$10,000",
      INCOME3 == 2 ~ "$12,500",
      INCOME3 == 3 ~ "$17,500",
      INCOME3 == 4 ~ "$22,500",
...
ggplot(df, aes(x = INCOME3, y = MENTHLTH)) +
